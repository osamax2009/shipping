...
const MonthPaymentCount = () => {
    ...
    const barLabels = dashboard?.monthly_payment_completed_report.map(
        (e, index) => index + 1
    );
    const barvalues = dashboard?.monthly_payment_completed_report.map(
        (e) => e.total_amount
    );
    const barData = {
        labels: barLabels,
...
export default MonthPaymentCount;
